...
class PriceContext:
    ...
    # None = 無當日大盤資料 → 無法排除 Strong Bear，保守不放行 BUY（docs/12 Phase 1）
...
def decide_entry(
    ...
    if ctx.market_trend_score is None:
        gates.append("無當日大盤資料，無法排除 Strong Bear")
    elif ctx.market_trend_score <= ef["strong_bear_market_score"]:

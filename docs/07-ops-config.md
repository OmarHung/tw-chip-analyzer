...
> 法人與 TPEx 報表留上線緩衝）。跑完後檢查行情／法人的 TWSE+TPEx 合計筆數是否達 `completeness`
> 門檻；**不完整**、或因手動回補／系統頁腳本佔用單飛鎖而**根本沒跑成**，都會每
> `retry.interval_minutes`（15 分）重試一次，直到 `retry.deadline_hour:deadline_minute`（18:00）為止，
> 到點仍不完整才放棄。「今天」一律以 `schedule.timezone` 換算，不依賴主機時區。
>
> **晚間信用補抓**（`schedule.credit`，2026-09-15 起）：融資券（TWSE+TPEx）與借券 TWT93U 交易所
> **晚間才公布**——線上 09-08 起每日首次入庫皆在 20:53～23:20、09-15 當日 19:37 仍查無資料，
> 16:00 的 EOD 從來抓不到（原本每天都重試到 18:00 才放棄，且推播必帶「不完整」警示）。故拆出獨立
> 排程：平日 **20:00** 起只抓這三張表（未公布時只是三個 GET，不重算），融資券/借券筆數達
> `credit.completeness` 門檻（或有新列）才重建特徵 + 落地訊號，每 15 分重試至 **23:30**。
> **Telegram 推播移到這段之後**；當日無行情（假日）則整段略過。借券未公布時端點回 `stat=OK` 但 0 筆、
> 不拋錯，只有筆數門檻抓得到。手動：`python -m app.jobs.scheduler --credit [--date YYYY-MM-DD]`。
...
> | `schedule.eod.completeness / retry`、`schedule.credit.*` | EOD 完整性檢查（行情/法人最低筆數）與重試節流；晚間信用補抓的起跑時間、融資券/借券最低筆數與截止時間（不完整或忙碌跳過時，每 N 分鐘重試至截止時間） |
> | `notify.telegram` | 每日推播「新進 BUY / AVOID」（`app.jobs.notify_signals`，晚間信用補抓〔融資券/借券〕完成後自動送；到截止時間仍不完整也會送，但附警示〔含 EOD 不完整原因〕；補算過去日期不送）。「新進」＝與該檔回看窗內最近一筆快照的 action 不同。**可在 `/settings`「Telegram 推播」卡片設定**（token、chat id〔可按「偵測」自動列出〕、啟用、推播類型、每類檔數、原因條數、最低成交金額；存 DB `notify_setting`，欄位 NULL＝沿用 `.env` 的 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` 與此 YAML；token 對外一律遮蔽、稽核 log 只記欄位名、httpx 請求 log 已遮蔽 URL 內的 token；寫入需 `OPS_API_KEY`）。`lookback_days`／分段長度／逾時重試只在 YAML。推播失敗只記 log，不影響 EOD。**純通知層**，不在 `data_version` 區塊內 |
...
> 不再像先前直接跳過整天；晚間信用補抓同理，最晚到 23:30）。寫入型操作需 `OPS_API_KEY`。
